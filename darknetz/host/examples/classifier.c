...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile_o, int *gpus, int ngpus, int clear, bool fl)
{
        ...
        while(get_current_batch(net) < net->max_batches || net->max_batches == 0) {
                ...
                endu = usage.ru_utime;
                ends = usage.ru_stime;
                printf("user CPU start: %ld.%ld; end: %ld.%ld\n", startu.tv_sec, startu.tv_usec, endu.tv_sec, endu.tv_usec);
                printf("kernel CPU start: %ld.%ld; end: %ld.%ld\n", starts.tv_sec, starts.tv_usec, ends.tv_sec, ends.tv_usec);
                printf("Max: %ld  kilobytes\n", usage.ru_maxrss);
                fprintf(output_file, "user CPU start: %ld.%ld; end: %ld.%ld\n", startu.tv_sec, startu.tv_usec, endu.tv_sec, endu.tv_usec);
                fprintf(output_file, "kernel CPU start: %ld.%ld; end: %ld.%ld\n", starts.tv_sec, starts.tv_usec, ends.tv_sec, ends.tv_usec);
                        ...
                        const char *weightfile, const char *filename, int top)
{
    ...
    // 3) 입출력 경로 준비
    char cfg_copy[256];
    strncpy(cfg_copy, cfgfile, sizeof(cfg_copy));
    cfg_copy[sizeof(cfg_copy)-1] = '\0';
    strtok(cfg_copy, "/.");
    char *model_tag = strtok(NULL, "/.");
    char output_dir[PATH_MAX];
    snprintf(output_dir, sizeof(output_dir),
             "/media/results/predict_%s_pps%d_ppe%d.txt",
             model_tag, partition_point1+1, partition_point2);
    ...
    while (1) {
        ...
        fprintf(stderr, "%s: Predicted in %.6f seconds.\n", input_path, elapsed);
